...
func newConnectDialFunc(target string, connectSSL bool, dialer netutil.Dialer) netutil.DialerFunc {
	return func(ctx context.Context, network, address string) (conn net.Conn, err error) {
		// TODO: make connect sample.
		// TODO: make httptrace callbacks called correctly.
		if network != "tcp" {
		...
		// NOTE: any logic for CONNECT request can be easily added via hooks.
		err = req.Write(conn)
		if err != nil {
			err = errors.WithStack(err)
			return
		}
		// NOTE: according to RFC 2817 we can send origin at that moment and not wait
